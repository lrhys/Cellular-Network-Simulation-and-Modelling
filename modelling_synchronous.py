...
class Simulator:
    ...
    def run_simulation(self, max_steps=100, iterations=1, save_output=False):
        results = []
        detailed_results = []
        ...
        for i in tqdm(range(1, iterations + 1)):
            ...
            # while self.future_event_list and self.sim_clock < max_time:
            ...
            for j in tqdm(range(1, max_steps + 1)):
                ...
                if save_output:
                    ...
                    detailed_results.append([i , j, (self.blocked_calls / self.total_number_of_calls) * 100, (self.dropped_calls / self.total_number_of_calls) * 100])
        ...
        if save_output:
            ...
            self.simulation_results.append([df['Percentage of blocked calls'].mean(), df['Percentage of dropped calls'].mean()])
        ...
        if save_output and debug:
            ...
            stats_df = pd.DataFrame(detailed_results, columns=['Iteration No.', 'Step No.', 'Percentage of blocked calls', 'Percentage of dropped calls'])
            ...
            self.simulation_stats_dfs.append(stats_df)
...
if __name__ == '__main__':
    ...
    # Logging setup
    logging.basicConfig(filename=f'simulation_report_{steps}Steps_{iterations}Iterations_{samples}Samples_{no_of_channels}Channels_{no_of_reserved_channels}Reserved.log', filemode='w', format='%(levelname)s - %(message)s', level=logging.INFO)
    ...
    for i in tqdm(range(1, samples + 1)):
        ...
        simulator.run_simulation(max_steps=steps, iterations=iterations, save_output=True)
        logging.info(f"----------------------Ended simulation successfully----------------------")

    # pool.shutdown(wait=True)

    logging.info(f"----------------------Saving simulation results to CSV----------------------")
    df = pd.DataFrame(simulator.simulation_results, columns=['Percentage of blocked calls (Sample Average)', 'Percentage of dropped calls (Sample Average)'])
    ...
    # Save simulation stats for all samples
    if debug:
        simulation_stats_all_samples = pd.concat(simulator.simulation_stats_dfs)
